...
class Simple_Regression_Bike_Prediction:
    ...
    try:
        ...
        def simple_regression_bike_prediction():
            ...
            csv_dataset = pd.read_csv('bike_sharing.csv')
            data_train = csv_dataset.sample(frac=0.8)
            data_test = pd.concat([csv_dataset, data_train]).drop_duplicates(keep=False)

            data_train.to_csv('bike_train_file.csv', header=True, index = None)
            data_test.to_csv('bike_test_file.csv', header=True, index = None)

            dataset = pd.read_csv('bike_train_file.csv')
            ...
            if accuracy > 0.8:
                ...
                pkl_file_name_path = 'Bike_Linear_Regression.pkl'
                ...
                ds_testData = pd.read_csv('bike_test_file.csv')
