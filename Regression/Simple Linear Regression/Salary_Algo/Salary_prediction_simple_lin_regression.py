...
class Simple_Regression:
    ...
    try:
        ...
        def simple_regression_using_pickle():
            ...
            csv_dataset = pd.read_csv('../salary_Datafile/Salary_Data.csv')
            data_train = csv_dataset.sample(frac=0.8)
            data_test = pd.concat([csv_dataset, data_train]).drop_duplicates(keep=False)

            data_train.to_csv('../salary_Datafile/train_file.csv', header=True, index = None)
            data_test.to_csv('../salary_Datafile/test_file.csv', header=True, index = None)

            dataset = pd.read_csv('../salary_Datafile/train_file.csv')
            ...
            pkl_file_name_path = '../Model/Linear_Regression.pkl'
            ...
            ds_testData = pd.read_csv('../salary_Datafile/test_file.csv')
